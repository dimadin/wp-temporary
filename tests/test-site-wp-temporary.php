<?php
/**
 * Tests: Tests_Site_WP_Temporary class
 *
...
/**
 * Class with tests for multisite temporaries.
 *
 * @since 1.0.0
 */
class Tests_Site_WP_Temporary extends WP_UnitTestCase {
	/**
	 * Basic test of WP_Temporary.
	 *
	 * Based on core test Tests_Option_SiteTransient::test_the_basic()
	 *
	 * @since 1.0.0
	 */
	public function test_the_basics() {
		...
		$this->assertFalse( WP_Temporary::get_site( 'doesnotexist' ) );
		$this->assertTrue( WP_Temporary::set_site( $key, $value ) );
		$this->assertEquals( $value, WP_Temporary::get_site( $key ) );
		$this->assertFalse( WP_Temporary::set_site( $key, $value ) );
		$this->assertTrue( WP_Temporary::set_site( $key, $value2 ) );
		...
		$this->assertFalse( WP_Temporary::delete_site( $key ) );
	}

	/**
	 * Test saving serialized data.
	 *
	 * Based on core test Tests_Option_SiteTransient::test_serialized_data()
	 *
	 * @since 1.0.0
	 */
	...
	public function test_old_timeout_delete() {
		...
		$timeout          = '_site_temporary_timeout_' . $key;
		$temporary_option = '_site_temporary_' . $key;
		add_filter( 'site_option_' . $timeout, '__return_zero' );
	...
	/**
	 * Test if site temporary without timeout is stored in autoloaded options.
	 *
	 * Based on core test Tests_Option_SiteTransient::test_set_site_transient_is_not_stored_as_autoload_option()
	 *
	 * @since 1.0.0
	 */
	...
	public function test_without_cleaning() {
		...
		$this->assertFalse( get_site_option( '_site_temporary_' . $key ) );
	...
	public function test_with_cleaning() {
		...
		// Setting temporary should be true.
		$this->assertTrue( WP_Temporary::set_site( $key, $value, 5 ) );

		// Direct retrieval of temporary value should return set value.
		$this->assertEquals( get_site_option( '_site_temporary_' . $key ), $value );

		// Direct retrieval of temporary timeout should be integer and greater than current time.
		$raw_key2_timeout_before_sleep = get_site_option( '_site_temporary_timeout_' . $key );
		$this->assertTrue( is_int( $raw_key2_timeout_before_sleep ) );
		$this->assertGreaterThan( time(), $raw_key2_timeout_before_sleep );

		// Getting of temporary should return set value.
		$this->assertEquals( WP_Temporary::get_site( $key ), $value );
